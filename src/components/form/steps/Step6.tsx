...
export const schema6 = z.object({
  enviornment: z.array(z.object({
    stock: z.enum(['true', 'false']),
    mainStreet: z.enum(['true', 'false']),
    signBoard: z.array(z.object({
    ...
    <>
      ...
      <div className={'grid grid-cols-2 gap-[12px]'}>
              ...
              <FormControl>
                <RadioGroup className={'col-span-2 grid grid-cols-2 gap-[12px]'}
                  ...
                  <FormItem className="flex col-span-1 items-center space-x-2">
                    <FormControl><RadioGroupItemSecondary label={'غیر بورس'}
                                                          value="false" /></FormControl>
                  </FormItem>
                  <FormItem className="flex items-center space-x-2">
                    <FormControl><RadioGroupItemSecondary label={'بورس'}
            ...
            <FormItem className={'col-span-2 grid grid-cols-2 gap-[12px]'}>
              ...
              <FormControl>
                <RadioGroup className={'col-span-2 grid grid-cols-2 gap-[12px]'}
                            onValueChange={field.onChange} defaultValue={'true'}>
                  <FormItem className="flex items-center space-x-2">
                    <FormControl><RadioGroupItemSecondary label={'خیابان فرعی'}
                                                          value="false" /></FormControl>
                    {/*<FormLabel>No</FormLabel>*/}
                  </FormItem>
                  <FormItem className="flex items-center space-x-2">
                    <FormControl><RadioGroupItemSecondary label={'خیابان اصلی'}
                                                          value="true" /></FormControl>
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="p-4 grid col-span-2 space-y-[12px] grid-cols-2 mb-4 pt-3 relative">
            {fields.length > 1 && (
              <Button
                type="button"
                size="icon"
              ...
              render={({ field }) => (
                <FormItem className={'col-span-2'}>
                  ...
                  >
                    <FormControl>
                      <SelectTrigger
                        className={'w-full h-[64px] border border-[#E4E4E4] bg-[#F9F9F9]'}>
                        <SelectValue placeholder="نوع تابلو سردرب"
            ...
            {form.watch('enviornment')[0].signBoard[0].type !== 'none' && <div className="flex gap-4 col-span-2">
              <FormField
                control={form.control}
                  ...
                  <FormItem className="flex-1">
                    ...
                    <FormControl>
                      <Input
                        ...
                        placeholder={'ارتفاع'} type={'number'}  {...field}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex-1">
                    ...
                    <FormControl>
                      <Input startIcon={<span className={'text-lg text-[#babcbe]'}>متر</span>}
                             placeholder={'عرض'} type="number" {...field}
                             onChange={(e) => field.onChange(+e.target.value)} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
            ...
            {form.watch('enviornment')[0].signBoard[0].type !== 'none' && <FormItem className={'col-span-1'}>
              <FormLabel htmlFor={'signboard-attachment'}
                         className={'w-full flex items-center gap-[18px] bg-[#EEEEEE] rounded-[20px] justify-center py-[14px]'}>
                <Camera />
                تصویر ضمیمه
                <InputSecondary
