...
export function Step10() {
            ...
            <Drawer key={i}>
                <DrawerTrigger asChild>
                    <button
                        ...
                        className="w-full px-3 py-2 text-primary flex items-center gap-2 border border-primary rounded-[20px] justify-between bg-white shadow-sm"
                    >
                ...
                <DrawerContent className="p-4">
                    <img src={previewImg ?? ''} alt="Preview" className="max-h-[80vh] mx-auto rounded-lg shadow"/>
                </DrawerContent>
            </Drawer>
        ))
    ...
    const Row = ({label, value}: { label?: string; value?: string }) => (
        <div
            className="flex justify-between border-b-[0.5px] border-[#B6B6B6] font-medium min-h-[51px] items-center py-1">
    ...
    return (
        <div className="space-y-6 text-right">
            ...
            <Section title="توضیحات فروشگاه">
                ...
                <Row label="سابقه همکاری با پنتل:" value={`${data.storeDescription?.cooperationHistory} سال`}/>
                <Row label="نوع فروش:"
                     value={data.storeDescription?.sellerType === 'wholesaler' ? 'عمده فروش' : 'خرده فروش'}/>
                <Row label="زمینه فعالیت:" value={data.foa?.join(', ')}/>
                <Row label="نحوه خرید:" value={data.purchaseMethod === 'direct' ? 'مستقیم' : 'غیر مستقیم'}/>
            ...
            <Section title="تصاویر فروشگاه">
                ...
                <div className={'mt-3'}> {data.externalImages?.map((pair, i) => (
                    <div key={i} className="flex gap-2">{renderImages(pair)}</div>
                ))}</div>
                <Row label="تصاویر داخلی فروشگاه:"/>
                <div className={'mt-3'}>{data.internalImages?.map((pair, i) => (
                    <div key={i} className="flex gap-2">{renderImages(pair)}</div>
                ))}</div>
            </Section>
            ...
            <div className="border p-4 rounded-xl space-y-3 mt-6 border-[#E4E4E4]">
                <div className={'flex items-center justify-between h-[51px] border-b border-[#E4E4E4]'}>
                    <p className="font-bold text-[20px]">احراز هویت کارشناس</p>
                ...
                <Button variant="outline"
                        className="flex w-full text-lg font-medium items-center justify-between text-primary border border-primary">
